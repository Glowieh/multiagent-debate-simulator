...
class RecordingAgent:
    def __init__(self, responses: list[AIMessage] | None = None) -> None:
        ...
        self.invoke_summary_calls: list[dict[str, Any]] = []
    ...
    def invoke_summary(self, topic: str, context: str) -> str:
        self.invoke_summary_calls.append({"topic": topic, "context": context})
        return "summary"
...
def recording_agents(
    ...
    monkeypatch.setattr("debate.agents.instances.get_debater_red", lambda: red)
    monkeypatch.setattr("debate.agents.instances.get_debater_green", lambda: green)
    monkeypatch.setattr("debate.agents.instances.get_summarizer", lambda: summarizer)
        ...
        **build_initial_state("Test topic"),
        "turn_messages": [
            AIMessage(
                content="",
                tool_calls=[
                    {"name": "wikipedia_search", "args": {"query": "AI"}, "id": "1"}
                ],
            ),
            AIMessage(content="forced final reply"),
        ],
    }
...
def test_final_turn_content_walks_back_for_non_empty_ai_message() -> None:
    ...
    assert final_turn_content(turn_messages) == "actual reply"
...
def test_debater_red_agent_node_does_not_reset_wikipedia_turn_on_later_tool_call(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tool_response = AIMessage(
        content="",
...
def test_debater_red_agent_node_force_final_when_loops_exhausted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    red = RecordingAgent([AIMessage(content="forced final")])
    monkeypatch.setattr("debate.agents.instances.get_debater_red", lambda: red)
    state: DebateState = {
        **build_initial_state("Test topic"),
        "turn_messages": [
    ...
    assert red.calls[0]["kwargs"]["force_final"] is True


def test_summarizer_node_commits_summary_message(
    recording_agents: tuple[RecordingAgent, RecordingAgent, RecordingAgent],
) -> None:
    ...
    assert summarizer.invoke_summary_calls == [
        {
            "topic": "Test topic",
            "context": "Red (turn 1): Red point.\nGreen (turn 1): Green point.",
        }
    ]
